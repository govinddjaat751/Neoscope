...
childModeButton.innerHTML = "Child Mode: ON"; // Initial state
...
childModeButton.addEventListener('click', () => {
    ...
    childModeButton.innerHTML = isChildMode ? "Child Mode: Off" : "Child Mode: On"; // Change text

    if (isChildMode) {
        // Redirect to another page when Child Mode is turned Off
...
quizButton.innerHTML = "Start Quiz"; // Button text
quizButton.style.position = "absolute";
quizButton.style.top = "12px"; // Align it with Child Mode button
quizButton.style.right = "280px"; // Position it to the left of Child Mode button
quizButton.style.backgroundColor = "rgba(0, 0, 0, 0.8)";
...
quizButton.style.fontSize = "18px";
quizButton.style.transition = "background-color 0.3s";
quizButton.style.zIndex = 10;

// Add hover effect for the Start Quiz button
quizButton.onmouseover = () => {
    quizButton.style.backgroundColor = "rgba(255, 255, 255, 0.2)";
};
quizButton.onmouseout = () => {
    quizButton.style.backgroundColor = "rgba(0, 0, 0, 0.8)";
};

// Append the Start Quiz button to the document body
document.body.appendChild(quizButton);

// Quiz button functionality: redirect when clicked
quizButton.addEventListener('click', () => {
    window.location.pathname = "solarsystemquiz.html"; // Change this to the desired quiz page
});
...
window.addEventListener('click', (event) => {
    ...
    if (intersects.length > 0) {
        ...
        if (object === sun) {
            window.location.href = "sunchildmode.html";
        } else {
            const planetData = planets.find(p => p.mesh === object);
        
            // Redirect to the corresponding planet page with "child" appended to the planet name
            window.location.href = `${planetData.name.toLowerCase()}child.html`;
        
            // Optionally, you could also update the information display here if needed
            infoDiv.innerHTML = `
                <strong>${planetData.name} Child</strong><br>
